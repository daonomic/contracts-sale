...
const Sale = artifacts.require("SimplePriceBasedCappedSale.sol");
...
contract('PriceBasedCappedSale', accounts => {
  let token;
  let paymentToken;
  let sale;
  ...
  it("should sell 10 tokens for 1 ETH and return nothing", async () => {
  ...
  it("should sell 20 tokens for 10 ETH and return change", async () => {
    await verifyBalanceChange(accounts[0], tokens(2), () => sale.sendTransaction({value: tokens(10), gasPrice: 0}));
    assertEq(await token.balanceOf(accounts[0]), tokens(20))
  });
